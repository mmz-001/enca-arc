...
mod tests {
    ...
    fn test_get_median_value() {
        let get_point =
            |value| EvaluatedPoint::new(DVector::zeros(2), &DVector::zeros(2), 1.0, &mut |_: &_| value).unwrap();
        let get_generation = |values: &[f64]| values.iter().map(|v| get_point(*v)).collect::<Vec<_>>();

        assert_eq!(2.0, get_median_value(&get_generation(&[0.0, 1.0, 2.0, 3.0, 4.0])));
        assert_eq!(1.5, get_median_value(&get_generation(&[0.0, 1.0, 2.0, 3.0])));
    }
    ...
    fn update_shared(mode: Mode, expected_best: f64, expected_median: f64) {
        ...
        let mut update = |h: &mut History| {
            let mut generation = [
                EvaluatedPoint::new(DVector::zeros(4), &DVector::zeros(4), 0.0, &mut function).unwrap(),
                EvaluatedPoint::new(DVector::zeros(4), &DVector::zeros(4), 0.0, &mut function).unwrap(),
            ];
    ...
    fn update_and_test(
        ...
        assert_eq!(history.current_best_individual().unwrap().value, expected_current_best);
        assert_eq!(history.overall_best_individual().unwrap().value, expected_overall_best);
    }
