...
impl CovarianceMatrix {
    ...
    pub fn set_cov(&mut self, new: SquareMatrix<f64>, update_eigen: bool) -> Result<(), PosDefCovError> {
        self.cov = new;
        // Ensure symmetry
    ...
    fn update_eigendecomposition(&mut self) -> Result<(), PosDefCovError> {
        let mut eigen = nalgebra::SymmetricEigen::try_new(self.cov.clone(), 1e-20, 0).ok_or(PosDefCovError)?;
...
mod tests {
    ...
    fn test_update_eigendecomposition() {
        ...
        cov.set_cov(SquareMatrix::from_iterator(2, 2, [3.0, 1.5, 1.5, 2.0]), false)
            .unwrap();
        ...
        let reconstructed = cov.eigenvectors.clone() * cov.sqrt_eigenvalues.pow(2) * cov.eigenvectors.transpose();
        ...
        cov.set_cov(SquareMatrix::from_iterator(2, 2, [3.0, 5.0, 5.0, 2.0]), false)
            .unwrap();
