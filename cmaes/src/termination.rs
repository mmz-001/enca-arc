    ...
    pub(crate) fn check_termination_criteria(self) -> Vec<TerminationReason> {
        ...
        if self.history.best_function_values().len() >= past_generations_a {
            ...
            if let (Some(first_median_value), Some(best_median_value)) = (
                ...
                let tol_fun_rel_range = tol_fun_rel_option * (first_median_value - best_median_value).abs();
        ...
        if (0..dim).all(|i| (sigma * cov[(i, i)]).abs() < tol_x) && path_c.iter().all(|x| (sigma * *x).abs() < tol_x) {
        ...
        let tol_stagnation_generations = get_tol_stagnation_generations(tol_stagnation_option, self.state.generation());
        ...
        if let Some(tol_stagnation_generations) = tol_stagnation_generations {
                ...
                && self.history.median_function_values().len() >= tol_stagnation_generations
            {
                ...
                let did_values_regress = |values: &VecDeque<f64>| {
                    ...
                    mode.is_better(Data::new(last_values).median(), Data::new(first_values).median())
                };
...
fn get_tol_stagnation_generations(tol_stagnation_option: usize, current_generation: usize) -> Option<usize> {
...
mod tests {
    ...
    fn get_dummy_generation(function_value: f64) -> Vec<EvaluatedPoint> {
        (0..100)
            .map(|_| {
                EvaluatedPoint::new(DVector::zeros(DIM), &DVector::zeros(DIM), 1.0, &mut |_: &DVector<
                    f64,
                >| {
                    function_value
                })
        ...
        run_termination_test(
            ...
            |state| {
                *state.mut_sigma() = 1e6;
                state
                    .mut_cov()
                    .set_cov(SquareMatrix::from_iterator(DIM, DIM, [0.01, 0.0, 0.0, 1e6]), true)
                    .unwrap();
            },
            |_| {},
            |_| {},
    ...
    fn test_check_termination_criteria_tol_fun() {
        ...
        run_termination_test(
            ...
            |_| {},
            |history| {
                history.mut_best_function_values().extend(vec![historical_best; 100]);
                history.mut_best_function_values().push_front(most_recent_best);
            },
    ...
    fn test_check_termination_criteria_tol_condition_cov() {
        ...
        run_termination_test(
            ...
            |state| {
                state
                    .mut_cov()
                    .set_cov(SquareMatrix::from_iterator(2, 2, [0.99, 0.0, 0.0, 1e14]), true)
                    .unwrap();
            },
            |_| {},
            |_| {},
