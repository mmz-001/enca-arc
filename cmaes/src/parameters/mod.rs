...
impl Parameters {
    ...
    pub(crate) fn new(options: &CMAESOptions, seed: u64, termination: TerminationParameters) -> Self {
        ...
        let cmu =
            (1.0 - c1).min(a_cov * (mu_eff - 2.0 + 1.0 / mu_eff) / ((dim as f64 + 2.0).powi(2) + a_cov * mu_eff / 2.0));
