...
//! calling [`CMAES::run`] or [`CMAES::run_parallel`].
//!
//! ```no_run
...
/// A type that handles algorithm iteration and printing of results. Use [`CMAESOptions`]
/// to create a `CMAES`.
///
/// # Type parameters
///
...
impl<F> CMAES<F> {
    ...
    pub fn generations_per_eigen_update(&self) -> usize {
        ...
        (self.state.evals_per_eigen_update(&self.parameters) as f64 / self.parameters.lambda() as f64).ceil() as usize
    }
    ...
    pub fn print_initial_info(&self) {
        ...
        let title_string = format!("{:^7} | {:^19} | {:^8}", "Gen #", "Best function value", "Sigma",);
    ...
    pub fn print_final_info(&self, termination_reasons: &[TerminationReason]) {
        ...
        if let Some(overall) = overall_best {
...
impl<F: ObjectiveFunction> CMAES<F> {
    ...
    /// more control over termination is needed.
    ...
    fn sample(&mut self) -> Result<Vec<EvaluatedPoint>, InvalidFunctionValueError> {
        ...
        let individuals =
            self.sampler
                .sample(&self.state, self.parameters.mode(), self.parameters.parallel_update())?;
    ...
    /// iteration can be performed manually if finer control is needed.
...
impl<F: ParallelObjectiveFunction> CMAES<F> {
    ...
    fn sample_parallel(&mut self) -> Result<Vec<EvaluatedPoint>, InvalidFunctionValueError> {
        let individuals =
            self.sampler
                .sample_parallel(&self.state, self.parameters.mode(), self.parameters.parallel_update())?;
...
mod tests {
    ...
    fn test_get_best_individuals() {
        let mut cmaes = CMAESOptions::new(vec![0.0; 10], 1.0).build(dummy_function).unwrap();
    ...
    fn test_immediate_termination() {
        ...
        let mut cmaes = CMAESOptions::new(vec![0.0; 10], 1.0).build(function).unwrap();
