...
impl CMAESOptions {
    ...
    /// Changes the bounds from the default value. Vector length must match the number of dimensions.
    ...
    pub fn enable_printing(mut self, min_gap_evals: impl Into<Option<usize>>) -> Self {
        self.print_gap_evals = min_gap_evals.into();
        self
    }
...
mod tests {
    ...
    fn test_build() {
        ...
        assert!(CMAESOptions::new(vec![1.0; 5], 1.0).build(dummy_function).is_ok());
        assert!(matches!(
        ...
        assert!(matches!(
            CMAESOptions::new(vec![1.0; 5], 1.0).cm(2.0).build(dummy_function),
            Err(InvalidOptionsError::Cm),
        ));
        assert!(matches!(
            CMAESOptions::new(vec![1.0; 5], 1.0).cm(-1.0).build(dummy_function),
            Err(InvalidOptionsError::Cm),
        ));
        assert!(matches!(
            CMAESOptions::new(vec![1.0; 5], 1.0).cm(f64::NAN).build(dummy_function),
            Err(InvalidOptionsError::Cm),
        ));
        assert!(matches!(
            CMAESOptions::new(vec![1.0; 5], 1.0)
