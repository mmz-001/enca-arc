...
fn run_test_no_effect<F: Fn(TerminationReason) -> bool + Clone>(check_reason: F) {
    ...
    let function = |x: &DVector<f64>| 1e-8 + (2.0 * x[0] - x[1]).abs().powf(1.5) + (2.0 - x[1]).powi(2);
    run_test(
        function,
...
#[test]
fn test_no_effect() {
    run_test_no_effect(|r| matches!(r, TerminationReason::NoEffectAxis | TerminationReason::NoEffectCoord,));
}
