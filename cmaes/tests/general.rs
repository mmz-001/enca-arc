    ...
    for _ in 0..TEST_REPETITIONS {
        ...
        match mode {
            Mode::Maximize => {
                ...
                modified_function = Box::new(move |x: &DVector<f64>| -objective_function.evaluate(x));
        ...
        if !result.reasons.iter().any(|r| *r == TerminationReason::FunTarget) {
...
fn test_ellipsoid() {
    let run_test_ellipsoid = |dim, pop_size_mult, max_avg_evals, max_failures, weights| {
        ...
        run_test(ellipsoid, Mode::Minimize, options, max_avg_evals, max_failures);
    };
...
fn test_rastrigin() {
    let run_test_rastrigin = |dim, pop_size_mult, max_avg_evals, max_failures, weights| {
        ...
        run_test(rastrigin, Mode::Minimize, options, max_avg_evals, max_failures);
    };
...
fn test_cigar() {
    let run_test_cigar = |dim, pop_size_mult, max_avg_evals, max_failures, weights| {
        ...
        run_test(cigar, Mode::Minimize, options.clone(), max_avg_evals, max_failures);
    };
...
mod consistent {
    ...
    fn test_api_usage() {
        ...
        let _ = CMAESOptions::new(vec![0.0; 5], 1.0)
            ...
            .unwrap();
        let _ = CMAESOptions::new(vec![0.0; 5], 1.0).build(non_static_function).unwrap();
